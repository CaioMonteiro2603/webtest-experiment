...
public class BrasilAgritest {
    ...
    public void testInvalidLogin() {
        ...
        WebElement errorMessage = wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector(".text-red-500, .text-red-600, .text-error, .text-danger, .alert-danger")));
        Assertions.assertTrue(errorMessage.getText().contains("Credenciais") || errorMessage.getText().contains("inválidas"));
    }
    ...
    public void testNavigationMenu() {
        ...
        WebElement equipmentItem = wait.until(ExpectedConditions.elementToBeClickable(
            By.xpath("//a[contains(@href,'/equip')]")));
        equipmentItem.click();
        
        wait.until(ExpectedConditions.urlContains("/equip"));
    ...
    public void testLogout() {
        ...
        WebElement logoutLink = wait.until(ExpectedConditions.elementToBeClickable(
            By.cssSelector("a[href*='logout'], [data-testid='logout-link'], .dropdown-item")));
        logoutLink.click();
        
        wait.until(ExpectedConditions.urlContains("/login"));
        WebElement loginForm = wait.until(ExpectedConditions.visibilityOfElementLocated(
            By.cssSelector("form, input[type='email'], input[name='email']")));
    ...
    public void testTableSorting() {
        ...
        WebElement nameHeader = wait.until(ExpectedConditions.elementToBeClickable(
            By.xpath("//th[contains(text(),'Nome') or contains(text(),'Name') or contains(@class,'sortable')]")));
        nameHeader.click();
    ...
    public void testFooterLinks() {
        ...
        WebElement privacyLink = wait.until(ExpectedConditions.elementToBeClickable(
            By.xpath("//a[contains(text(),'Política') or contains(text(),'Privacidade')]")));
        privacyLink.click();
