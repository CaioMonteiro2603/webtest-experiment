...
public class saucedemo {
    ...
    public void testSortingDropdown() {
        ...
        // Re-locate dropdown before each interaction to avoid stale element
        // Test Name (A to Z)
        ...
        // Test Name (Z to A)
        sortDropdown = new Select(wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".product_sort_container"))));
    ...
    public void testMenuNavigation() {
        ...
        List<WebElement> closeButton = driver.findElements(By.className("bm-cross-button"));
        if (!closeButton.isEmpty() && closeButton.get(0).isDisplayed()) {
            closeButton.get(0).click();
            wait.until(ExpectedConditions.invisibilityOf(closeButton.get(0)));
        }
        ...
        // Close menu
        closeButton = driver.findElements(By.className("bm-cross-button"));
        if (!closeButton.isEmpty() && closeButton.get(0).isDisplayed()) {
            closeButton.get(0).click();
            wait.until(ExpectedConditions.invisibilityOf(closeButton.get(0)));
        }
