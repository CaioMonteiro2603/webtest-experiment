...
public class conduit {
    ...
    private void login() {
        ...
        try {
            ...
            // Fallback: wait for any user profile link or home feed to confirm login
    ...
    void testUserRegistration() {
        ...
        try {
            ...
            // Fallback: check for settings link or home feed as confirmation
            WebElement settingsLink = wait.until(ExpectedConditions.visibilityOfElementLocated(SETTINGS_LINK));
            Assertions.assertTrue(settingsLink.isDisplayed(), "User should be logged in and see 'Settings' link after registration.");
        }
    }
    ...
    void testLoginAndLogout() {
        ...
        try {
            ...
            Assertions.assertTrue(errorMessage.getText().contains("is invalid") || errorMessage.getText().contains("email or password"), "Error message should be shown for invalid credentials.");
        } catch (TimeoutException e) {
            // Fallback: check for any error message
            List<WebElement> errors = driver.findElements(ERROR_MESSAGES);
            Assertions.assertFalse(errors.isEmpty(), "Error message should be shown for invalid credentials.");
        }
