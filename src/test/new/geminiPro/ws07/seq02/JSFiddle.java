...
public class JSFiddle {
    ...
    void testSignInModalAndInvalidLogin() {
        wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href*='login'], button[data-target*='login'], .login-btn, #login-button"))).click();
    ...
    void testFooterDocsLink() {
        ...
        WebElement docsLink = driver.findElement(By.cssSelector("a[href*='docs'], footer a[href*='docs'], a[title*='Documentation']"));
    ...
    private void enterCodeIntoPane(String paneType, String code) {
        // Find the editor panel by class name instead of iframe
        WebElement panel = wait.until(ExpectedConditions.presenceOfElementLocated(
            By.cssSelector(".panel-" + paneType + " .CodeMirror")));
        
        // Click to focus the editor
        ((JavascriptExecutor) driver).executeScript("arguments[0].CodeMirror.setValue(arguments[1]);", panel, code);
        
        // Trigger change event
        ((JavascriptExecutor) driver).executeScript("arguments[0].CodeMirror.save();", panel);
    }
}