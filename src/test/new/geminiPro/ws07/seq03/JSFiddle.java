...
public class JSFiddle {
    ...
    // iframe Locators
    private static final By HTML_PANEL_IFRAME = By.cssSelector("#editor-tabs-html + div iframe");
    private static final By CSS_PANEL_IFRAME = By.cssSelector("#editor-tabs-css + div iframe");
    ...
    void testSaveFunctionality() {
        ...
        Assertions.assertTrue(!newUrl.equals(BASE_URL) && !newUrl.equals(BASE_URL + "/"), "Saved URL should have a unique hash and not be the base URL.");
    }
    ...
    void testExternalHelpLink() {
        ...
        // Try alternative locator strategies for Help link
        try {
            wait.until(ExpectedConditions.elementToBeClickable(HELP_DROPDOWN)).click();
        } catch (Exception e) {
            // Try alternative xpath
            By altHelpLocator = By.xpath("//a[contains(text(),'Help')]");
            wait.until(ExpectedConditions.elementToBeClickable(altHelpLocator)).click();
        }
