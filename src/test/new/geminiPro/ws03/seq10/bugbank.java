...
public class bugbank {
    ...
    void testUserRegistration() {
        ...
        WebElement balanceToggle = wait.until(ExpectedConditions.elementToBeClickable(BALANCE_TOGGLE));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", balanceToggle);
        balanceToggle.click();
    ...
    void testLoginFunctionality() {
        ...
        // Wait for home page to load instead of checking URL
        wait.until(ExpectedConditions.visibilityOfElementLocated(WELCOME_TEXT));
        Assertions.assertTrue(driver.getCurrentUrl().endsWith("/home") || driver.findElement(WELCOME_TEXT).isDisplayed(), "Should be redirected to home page after login.");
    ...
    void testBankStatement() {
        // Wait for statement button to be clickable with longer timeout
        wait.withTimeout(Duration.ofSeconds(15)).until(ExpectedConditions.elementToBeClickable(STATEMENT_BUTTON)).click();
    ...
    void testLogout() {
        // Wait for logout button with longer timeout and scroll into view if needed
        WebElement logoutBtn = wait.withTimeout(Duration.ofSeconds(15)).until(ExpectedConditions.presenceOfElementLocated(LOGOUT_BUTTON));
