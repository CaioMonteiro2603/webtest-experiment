 ...
 * NOTE: The prompt provided a BASE_URL for a customer service form, but the functional
...
public class TAT {
    ...
    private void testExternalLink(By locator, String expectedDomain) {
        ...
        if (currentWindows.size() > 1) {
            currentWindows.remove(originalWindow);
            String newWindow = currentWindows.iterator().next();
            driver.switchTo().window(newWindow);
            
            if (expectedDomain.contains("twitter.com")) {
                try {
                    Thread.sleep(3000);
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
            ...
            Assertions.fail("No new window opened after clicking the link");
        }
    }
    ...
    void testInvalidLoginScenarios() {
        ...
        performLogin(VALID_USER, "badpassword");
    ...
    void testFullPurchaseWorkflow() {
        ...
        driver.findElement(By.id("postal-code")).sendKeys("13630-000");
    ...
    @Test
    @Order(5)
    void testBurgerMenuFunctionality() {
        performLogin(VALID_USER, PASSWORD);
