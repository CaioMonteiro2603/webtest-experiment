...
public class JSFiddle {
    ...
    public void baseLoads_HeaderFooterCoreAreasPresent() {
        ...
        // Look for footer more broadly - not necessarily just <footer> tag
        boolean footerPresent = present(By.tagName("footer")) || 
                               present(By.cssSelector(".footer, #footer, [class*='footer']"));
        Assertions.assertTrue(footerPresent, "Footer should be present");
    ...
    public void visitInternalLinks_OneLevelBelow() {
        ...
        for (String h : candidates) {
            ...
            try {
                ...
                // Scroll into view and wait for clickable
                WebElement web = linkOpt.get();
                ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", web);
                web = wait.until(ExpectedConditions.elementToBeClickable(web));
                web.click();
    ...
    public void externalLinksInFooterOrHeader_OpenOnDifferentDomain() {
        openBase();
        String baseHost = hostOf(driver.getCurrentUrl());
        // Look for external links more broadly - including social links, GitHub, etc.
        List<WebElement> anchors = driver.findElements(By.cssSelector("footer a[href], header a[href], nav a[href], a[target='_blank'], a[href*='github'], a[href*='twitter'], a[href*='facebook']"));
    ...
    public void optionalDropdowns_ChangeSelectionIfPresent() {
        ...
        // Scroll the select element into view first
        ...
        // Use JavaScript to change selection for options that might not be scrollable
        try {
            select.selectByIndex(1);
        } catch (ElementNotInteractableException e) {
            // If direct selection fails, use JavaScript
            ((JavascriptExecutor) driver).executeScript("arguments[0].selectedIndex = 1; arguments[0].dispatchEvent(new Event('change'));", selectEl);
        }
