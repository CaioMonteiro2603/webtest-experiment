...
public class DemoAUT {
    ...
    public void basePageLoadsAndHasFormElements() {
        ...
        boolean hasHeader = present(By.tagName("h1")) || present(By.tagName("h2")) || present(By.tagName("title")) || present(By.xpath("//*[contains(text(),'Katalon')]")) || present(By.xpath("//*[contains(text(),'Form')]"));
    ...
    public void fillFormWithValidDataAndSubmit() {
        ...
        if (!expectations.isEmpty()) {
        	WebElement el = wait.until(ExpectedConditions.elementToBeClickable(expectations.get(0)));
            el.click();
    ...
    public void html5RequiredFieldsPreventEmptySubmission() {
        ...
        boolean hasRequiredMessage = msgs.stream().filter(Objects::nonNull)
                .map(String::toLowerCase).anyMatch(s -> s.contains("fill") || s.contains("required") || s.contains("blank") || s.length() > 0);
        Assumptions.assumeTrue(msgs.stream().anyMatch(m -> m != null && !m.isEmpty()), "No native validation surfaced; skipping");
