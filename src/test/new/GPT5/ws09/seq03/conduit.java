...
public class conduit {
    ...
    public void openArticleFromGlobalFeed_DetailsVisible() {
        ...
        // Wait for feed to load first
        wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".feed-toggle, .article-preview, .article-meta")));
        try { Thread.sleep(1000); } catch (InterruptedException ignored) {} // Allow time for feed load
        ...
        if (firstArticle == null) {
            List<WebElement> candidates = all(By.cssSelector("a[href*='/article/']"));
            if (!candidates.isEmpty()) {
                firstArticle = candidates.get(0);
            }
        }
        ...
        WebElement title = first(By.cssSelector(".article-page h1, .article-content h1"));
    ...
    public void internalRoutes_OneLevelBelow_AreReachable() {
        ...
        for (String r : routes) {
            ...
            if (r.contains("login")) {
                ...
                String current = currentRoute();
                boolean validRedirectOrAccess = current.contains("settings") || current.contains("login");
                Assertions.assertTrue(validRedirectOrAccess,
                ...
                try { Thread.sleep(500); } catch (InterruptedException ignored) {}
                String current = currentRoute();
                boolean validRedirectOrAccess = current.contains("editor") || current.contains("login");
                Assertions.assertTrue(validRedirectOrAccess,
    ...
    public void footerExternalLinks_OpenAndMatchDomains() {
        ...
        List<WebElement> footerAnchors = driver.findElements(By.cssSelector("footer a[href], .footer a[href], footer a"));
        int checked = 0;
        for (Map.Entry<String, String> entry : expectedDomains.entrySet()) {
            String key = entry.getKey().toLowerCase();
            Optional<WebElement> link = footerAnchors.stream()
                    .filter(a -> {
    ...
    public void editorRoute_RequiresLogin_RedirectsOrBlocks() {
        ...
        boolean editorOrLogin = wait.until(d -> {
            try {
                d.findElement(By.tagName("body")); // Keep driver alive
                String curr = currentRoute();
                return curr.contains("editor") || curr.contains("login");
            } catch (Exception e) {
                return false;
            }
        });
