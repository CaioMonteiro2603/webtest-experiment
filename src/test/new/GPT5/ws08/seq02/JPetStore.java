...
public class JPetStore {
    ...
    private void openExternalAndAssert(By linkLocator, String expectedDomainFragment) {
        ...
        if (driver.getWindowHandles().size() > before.size()) {
            ...
            wait.until(d -> !d.getCurrentUrl().equals("about:blank") && d.getCurrentUrl().toLowerCase(Locale.ROOT).contains(expectedDomainFragment.toLowerCase(Locale.ROOT)));
    ...
    public void homePageLoads_CoreUIVisible() {
        ...
        WebElement catalogLink = firstDisplayed(By.partialLinkText("Catalog"), By.cssSelector("a[href='/catalog/']"), By.cssSelector("a[href*='/catalog']"));
    ...
    public void catalog_SearchIfPresent_NavigatesOrShowsResults() {
        ...
        if (search != null) {
            ...
            waitForReady();
            boolean navigated = driver.getCurrentUrl().contains("/search") || driver.findElements(By.cssSelector("table, .product, .item, .list")).size() > 0;
