...
public class JPetStore {
    ...
    public void footerExternalLinksOpen() {
        ...
        wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector("footer a[href], .footer a[href], #footer a[href]")));

        // Collect candidate external links (limit to 2 for stability)
        List<WebElement> anchors = driver.findElements(By.cssSelector("footer a[href], .footer a[href], #footer a[href]"));
        if (anchors.isEmpty()) {
            // Fallback to any link in footer area
        ...
        for (Map.Entry<String, WebElement> e : targets.entrySet()) {
            ...
            try {
                ...
                // Skip this link and try next one
                continue;
            }
        }
