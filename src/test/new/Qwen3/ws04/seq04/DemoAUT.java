...
public class DemoAUT {
    ...
    public void testInvalidFormSubmission() {
        ...
        try {
            ...
            // If .error-message is not found, try alternative selectors
            try {
                WebElement validationMessage = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".invalid-feedback")));
    ...
    public void testFormFieldsValidation() {
        ...
        List<WebElement> errorFields = driver.findElements(By.cssSelector(".error-field, .invalid-feedback"));
        if (errorFields.isEmpty()) {
            // Check for HTML5 validation
            WebElement firstNameField = driver.findElement(By.id("firstName"));
            String validationMessage = firstNameField.getAttribute("validationMessage");
            if (validationMessage != null && !validationMessage.isEmpty()) {
                assertTrue(true); // HTML5 validation is working
            } else {
                assertFalse(true); // Force fail if no validation found
            }
        } else {
    ...
    public void testFormNavigation() {
        ...
        List<WebElement> navLinks = driver.findElements(By.cssSelector("nav a, .nav a, .navigation a"));
        if (!navLinks.isEmpty()) {
        ...
        try {
            ...
            // If no form tag, check for form elements
