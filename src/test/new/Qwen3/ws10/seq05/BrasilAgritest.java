...
public class BrasilAgritest {
    ...
    public void testLoginPageLoad() {
        ...
        assertTrue(pageTitle.contains("Login") || pageTitle.contains("login") || pageTitle.contains("Entrar"), "Page title should contain Login");
    ...
    public void testInvalidCredentialsError() {
        ...
        assertTrue(errorMessage.getText().contains("Credenciais inválidas") || 
                   ...
                   errorMessage.getText().contains("Erro") ||
    ...
    public void testLoginFormValidation() {
        ...
        if (passwordError.isEmpty()) {
            passwordError = driver.findElements(By.cssSelector(".form-group:nth-child(2) .error-message"));
        }
        if (!passwordError.isEmpty()) {
    ...
    public void testResponsiveDesignElements() {
        ...
        // Check for proper header - try different selectors
        try {
            WebElement header = driver.findElement(By.cssSelector("header, .header, [role='banner']"));
            assertTrue(header.isDisplayed(), "Header should be displayed");
        } catch (NoSuchElementException e) {
            // Try to find any top-level navigation or brand element
            List<WebElement> topElements = driver.findElements(By.cssSelector(".navbar, .header, .top-bar, [role='banner']"));
            if (!topElements.isEmpty()) {
                assertTrue(topElements.get(0).isDisplayed(), "Top navigation element should be displayed");
            }
        }
        
        // Check for mobile menu elements if they exist
        try {
            WebElement mobileMenu = driver.findElement(By.cssSelector(".mobile-menu-toggle, [data-toggle='mobile-menu']"));
    ...
    public void testPageStructure() {
        ...
        // Check semantic HTML elements - be more flexible with selectors
        List<WebElement> headerElements = driver.findElements(By.tagName("header"));
        if (headerElements.isEmpty()) {
            headerElements = driver.findElements(By.cssSelector(".header, .navbar, .top-bar"));
        }
        assertTrue(headerElements.size() >= 1, "Should have at least one header/navigation element");
        
        List<WebElement> mainElements = driver.findElements(By.tagName("main"));
        if (mainElements.isEmpty()) {
            mainElements = driver.findElements(By.cssSelector(".main, .content, .container, .wrapper"));
        }
        assertTrue(mainElements.size() >= 1, "Should have at least one main content element");
        
        List<WebElement> footerElements = driver.findElements(By.tagName("footer"));
        if (footerElements.isEmpty()) {
            footerElements = driver.findElements(By.cssSelector(".footer, footer, [role='contentinfo']"));
        }
