...
public class parabank {
    ...
    public void testValidLogin() {
        ...
        WebElement welcomeMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("h1.title")));
    ...
    public void testExternalLink_Twitter() {
        ...
        try {
            ...
            Assertions.assertTrue(url.contains("twitter"), "Twitter link did not open correct domain.");
            driver.close();
            driver.switchTo().window(originalWindow);
        } catch (TimeoutException e) {
            Assertions.fail("Twitter link not found or not clickable");
        }
    }
    ...
    public void testExternalLink_Facebook() {
        ...
        try {
            ...
            driver.switchTo().window(newWindow);
            String url = driver.getCurrentUrl();
            Assertions.assertTrue(url.contains("facebook"), "Facebook link did not open correct domain.");
            driver.close();
            driver.switchTo().window(originalWindow);
        } catch (TimeoutException e) {
            Assertions.fail("Facebook link not found or not clickable");
        }
    }
    ...
    public void testExternalLink_LinkedIn() {
        ...
        try {
            ...
            driver.switchTo().window(newWindow);
            String url = driver.getCurrentUrl();
            Assertions.assertTrue(url.contains("linkedin"), "LinkedIn link did not open correct domain.");
            driver.close();
            driver.switchTo().window(originalWindow);
        } catch (TimeoutException e) {
            Assertions.fail("LinkedIn link not found or not clickable");
        }
    }
