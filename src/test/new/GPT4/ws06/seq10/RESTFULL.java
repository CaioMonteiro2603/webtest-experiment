...
public class RESTFULL {
    ...
    public void testHomeLoad() {
        ...
        Assertions.assertAll("Basic check",
            () -> Assertions.assertTrue(driver.getTitle().toLowerCase().contains("restful"), "Title should contain 'automation'"),
            () -> Assertions.assertTrue(driver.findElements(By.tagName("nav")).size() > 0, "Header or nav must be present"),
    ...
    public void testInternalLinksOneLevel() {
        ...
        for (int i = 0; i < links.size() && tested < 5; i++) {
            try {
    ...
    public void testExternalLinksFooter() {
        ...
        String baseHost = hostOf(BASE_URL);
        int tested = 0;
        java.util.List<WebElement> footerLinks = driver.findElements(By.cssSelector("footer a[href]"));
        if (footerLinks.isEmpty()) {
            footerLinks = driver.findElements(By.cssSelector("a[href]"));
        }
        ...
        if (tested == 0) {
            ...
            java.util.List<WebElement> allLinks = driver.findElements(By.cssSelector("a[href]"));
