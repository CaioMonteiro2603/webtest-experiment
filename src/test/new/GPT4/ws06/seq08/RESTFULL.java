...
public class RESTFULL {
    ...
    public void testHomePageLoads() {
        ...
        Assertions.assertTrue(heading.getText().toLowerCase().contains("shady meadows b&b"),
                "Hero title should contain 'Shady Meadows B&B'");
    }
    ...
    public void testNavRoomsPageLoads() {
        ...
        Assertions.assertEquals("Our Rooms", roomHeader.getText(), "Rooms section should be visible");
    }
    ...
    public void testRoomCardElementsPresent() {
        ...
        List<WebElement> cards = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector(".row .col-sm-7")));
        Assertions.assertFalse(cards.isEmpty(), "At least one room card should be present");

        WebElement firstCard = cards.get(0);
        WebElement title = firstCard.findElement(By.tagName("h3"));
        WebElement price = firstCard.findElement(By.tagName("p"));
    ...
    public void testBookThisRoomButtonExists() {
        driver.get(BASE_URL + "#rooms");
        List<WebElement> buttons = wait.until(ExpectedConditions.presenceOfAllElementsLocatedBy(By.cssSelector(".btn")));
        List<WebElement> bookButtons = buttons.stream()
                .filter(btn -> btn.getText().equals("Book this room"))
                .toList();
        Assertions.assertFalse(bookButtons.isEmpty(), "There should be at least one 'Book this room' button");
    ...
    public void testSubmitContactFormMissingRequiredFields() {
        ...
        WebElement submitBtn = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("#contact .btn-primary")));
        submitBtn.click();
    ...
    public void testSubmitContactFormValid() {
        driver.get(BASE_URL + "#contact");
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("#contact input[name='name']"))).sendKeys("John Doe");
        driver.findElement(By.cssSelector("#contact input[name='email']")).sendKeys("john@example.com");
        driver.findElement(By.cssSelector("#contact input[name='phone']")).sendKeys("1234567890");
        driver.findElement(By.cssSelector("#contact input[name='subject']")).sendKeys("Test Subject");
        driver.findElement(By.cssSelector("#contact textarea[name='description']")).sendKeys("Test message content.");

        WebElement submitBtn = driver.findElement(By.cssSelector("#contact .btn-primary"));
        submitBtn.click();
    ...
    public void testFooterFacebookLink() {
        driver.get(BASE_URL);
        WebElement fbLink = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("a[href*='facebook.com']")));
        String originalWindow = driver.getWindowHandle();
