...
public class saucedemo {
    ...
    private void resetAppState() {
        try {
            ...
            try {
                ...
                // If reset link is not visible, just close menu
                menuBtn.click();
            }
        } catch (TimeoutException e) {
            // If menu is not available, skip reset
        }
    }
    ...
    public void testAddAndRemoveFromCart() {
        login();
        WebElement firstAddBtn = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(@class,'btn_inventory') and contains(text(),'ADD TO CART')]")));
        firstAddBtn.click();
        WebElement cartBadge = driver.findElement(By.className("shopping_cart_badge"));
        Assertions.assertEquals("1", cartBadge.getText(), "Cart badge should show 1 after adding one item.");
        WebElement removeBtn = driver.findElement(By.xpath("//button[contains(@class,'btn_inventory') and contains(text(),'REMOVE')]"));
        removeBtn.click();
