...
public class JSFIDDLE {
    ...
    public void testNewFiddleButtonCreatesNewFiddle() {
        ...
        newButton.click();
        WebElement htmlPanel = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div#id_code_html, .CodeMirror")));
    ...
    public void testFrameworksDropdownOpens() {
        driver.get(BASE_URL);
        WebElement settingsBtn = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("button[data-label='Settings'], .sidebar-tabs button")));
        settingsBtn.click();
        WebElement frameworksDropdown = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("select#framework, select")));
    ...
    public void testSignInPageNavigation() {
        ...
        Assertions.assertTrue(driver.getCurrentUrl().contains("/user/login"), "URL should contain /user/login");
    ...
    public void testInvalidLoginShowsError() {
        ...
        WebElement loginButton = driver.findElement(By.cssSelector("form[action*='/login/'] button[type='submit'], button[type='submit']"));

        username.sendKeys("invalidUser");
        password.sendKeys("invalidPass");
        loginButton.click();

        WebElement errorMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".errorlist, .alert-danger, .error")));
        Assertions.assertTrue(errorMsg.getText().toLowerCase().contains("enter a correct username") || 
    ...
    public void testFooterTwitterLink() {
        ...
        WebElement twitterLink = wait.until(ExpectedConditions.elementToBeClickable(By.cssSelector("a[href*='twitter.com'], a[href*='x.com']")));
        twitterLink.click();
        ...
        wait.until(ExpectedConditions.urlContains("twitter.com")/*.or(ExpectedConditions.urlContains("x.com"))*/);
        Assertions.assertTrue(driver.getCurrentUrl().contains("twitter.com")/*.or(driver.getCurrentUrl().contains("x.com"))*/, "Should open Twitter in new tab");
