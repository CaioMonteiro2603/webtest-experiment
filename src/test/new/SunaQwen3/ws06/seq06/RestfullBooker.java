...
public class RestfullBooker {
    ...
    public void testInvalidLogin() {
        ...
        assertTrue(errorMessage.getText().contains("Invalid") || errorMessage.getText().contains("invalid"), "Error message should indicate invalid credentials");
    }
    ...
    public void testFooterExternalLinks() {
        ...
        assertTrue(footerLinks.size() > 0, "Footer should have at least one link");

        String originalWindow = driver.getWindowHandle();

        int socialLinksCount = 0;
        for (WebElement link : footerLinks) {
            String linkText = link.getText();
            if (linkText.toLowerCase().contains("twitter") || linkText.toLowerCase().contains("facebook") || linkText.toLowerCase().contains("linkedin")) {
                socialLinksCount++;
            }
        }
        assertTrue(socialLinksCount <= footerLinks.size(), "Social links should be subset of footer links");
