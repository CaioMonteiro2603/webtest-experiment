...
public class DemoAUT {
    ...
    private static final String EXPECTED_TITLE = "Demo AUT";
    ...
    public void testFormElementsArePresent() {
        ...
        Assertions.assertTrue(isElementPresent(By.xpath("//input[@name='gender']")), "Gender field should be present");
    ...
    public void testFormSubmissionWithInvalidEmail() {
        ...
        // Select hobby
        WebElement hobby = driver.findElement(By.xpath("//input[@value='music']"));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", hobby);
        wait.until(elementToBeClickable(hobby));
        hobby.click();

        // Select subject
        WebElement subject = driver.findElement(By.xpath("//input[@value='arts']"));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", subject);
        wait.until(elementToBeClickable(subject));
        subject.click();

        // Select profession
        WebElement profession = driver.findElement(By.xpath("//input[@value='teacher']"));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", profession);
        wait.until(elementToBeClickable(profession));
        profession.click();
    ...
    public void testRequiredFieldsValidation() {
        ...
        Assertions.assertTrue(
            !validationMessage.isEmpty() || validationMessage != null,
    ...
    public void testExternalLinksInFooter() {
        ...
        // Skip footer test if no footer exists
        try {
            wait.until(visibilityOfElementLocated(By.tagName("footer")));
        } catch (TimeoutException e) {
            Assertions.fail("Footer not found on page");
            return;
        }
