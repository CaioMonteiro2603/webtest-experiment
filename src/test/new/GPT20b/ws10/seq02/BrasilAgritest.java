...
public class BrasilAgritest {
    ...
    public void testLoginPageLoads() {
        ...
        assertTrue(actualTitle.contains("gestão") || actualTitle.contains("login") || 
                actualTitle.contains("gestao") || actualTitle.contains("admin") || 
                actualTitle.contains("dashboard") || actualTitle.contains("sistema") ||
                driver.findElements(By.id("email")).size() > 0 ||
                driver.findElements(By.name("email")).size() > 0 ||
                driver.findElements(By.cssSelector("input[type='email']")).size() > 0,
                "Page should either have relevant title or login form elements");
    ...
    public void testValidLogin() {
        ...
        wait.until(d -> d.getCurrentUrl().contains("/dashboard") ||
                ...
                !driver.findElements(By.cssSelector("a[href*='logout']")).isEmpty() ||
                !driver.findElements(By.cssSelector("button[data-testid*='logout']")).isEmpty() ||
        ...
        assertTrue(url.contains("/dashboard") || url.contains("/home") || url.contains("/profile") || 
                url.contains("/admin") || !driver.findElements(By.cssSelector("a[href*='logout']")).isEmpty() ||
                driver.getTitle().toLowerCase().contains("dashboard") || driver.getTitle().toLowerCase().contains("home") ||
                driver.getTitle().toLowerCase().contains("admin"),
                "After login, should be on dashboard/home/profile or have logout option");

        List<WebElement> logoutLinks = driver.findElements(By.cssSelector("a[href*='logout'], button[data-testid*='logout']"));
    ...
    public void testInvalidLogin() {
        ...
        try {
            WebElement errorMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(
                    By.cssSelector(".alert.alert-danger, .alert-danger, .error-message, .error, p.error, div.error, span.error, [role='alert']")));
            ...
            // Check if we're still on login page as indication of failed login
    ...
    public void testSortingDropdown() {
        ...
        List<WebElement> sortSelectEls = driver.findElements(By.cssSelector(
                "select[name='sort'], select[id='sort'], select[aria-label='Sort'], select"));
    ...
    private void performLogin(String email, String pass) {
        ...
        WebElement emailField = driver.findElements(By.id("email")).stream().findFirst()
                ...
                .orElseGet(() -> driver.findElements(By.cssSelector("input[type='email']")).stream().findFirst().orElse(null)));
        ...
        WebElement pwdField = driver.findElements(By.id("password")).stream().findFirst()
                ...
                .orElseGet(() -> driver.findElements(By.cssSelector("input[type='password']")).stream().findFirst().orElse(null)));

        if (emailField == null || pwdField == null) {
            Assertions.fail("Could not locate email or password field");
        }
    ...
    private void ensureLoggedIn() {
        if (driver.findElements(By.cssSelector("a[href*='logout']")).isEmpty() &&
            driver.findElements(By.cssSelector("button[data-testid*='logout']")).isEmpty() &&
            !driver.getCurrentUrl().contains("dashboard") && !driver.getCurrentUrl().contains("home") && !driver.getCurrentUrl().contains("admin")) {
            performLogin(USER_EMAIL, PASSWORD);
            wait.until(d -> d.getCurrentUrl().contains("/dashboard") ||
                    d.getCurrentUrl().contains("/home") ||
                    d.getCurrentUrl().contains("/profile") ||
    ...
    private void navigateToInventory() {
        ...
        if (!itemsLinks.isEmpty()) {
            try {
                ...
                ((JavascriptExecutor)driver).executeScript("arguments[0].click();", itemsLinks.get(0));
            }
        } else {
            ...
            if (!inventoryLinks.isEmpty()) {
                try {
                    ...
                    ((JavascriptExecutor)driver).executeScript("arguments[0].click();", inventoryLinks.get(0));
                }
            } else {
        ...
        wait.until(ExpectedConditions.presenceOfElementLocated(
                By.cssSelector(".item-card, .product-card, .grid-item, .inventory-item")
        ));
    }

    private void openBurgerMenu() {
        List<WebElement> burgerButtons = driver.findElements(By.cssSelector(
                "button[aria-label='Menu'], .burger-menu, .hamburger, .nav-toggler, button[aria-expanded]"));
        ...
        try {
            ...
            ((JavascriptExecutor)driver).executeScript("arguments[0].click();", burgerButtons.get(0));
        }
    }

    private String getFirstItemName() {
        List<WebElement> items = driver.findElements(By.cssSelector(
                ".item-card .item-title, .product-card .product-title, .grid-item .title, .inventory-item .item-name"));
