...
public class JSFiddle {
    ...
    public void testFooterSocialLinks() {
        navigateToBase();
        List<WebElement> links = driver.findElements(By.xpath("//a[contains(@href,'twitter.com') or contains(@href,'x.com') or contains(@href,'facebook.com') or contains(@href,'linkedin.com') or contains(@href,'github.com')]"));
        Assertions.assertFalse(links.isEmpty(), "No social links found in footer.");

        String originalWindow = driver.getWindowHandle();
        for (int i = 0; i < links.size(); i++) {
            List<WebElement> currentLinks = driver.findElements(By.xpath("//a[contains(@href,'twitter.com') or contains(@href,'x.com') or contains(@href,'facebook.com') or contains(@href,'linkedin.com') or contains(@href,'github.com')]"));
            ...
            try {
                ...
                for (String handle : handles) {
                    if (!handle.equals(originalWindow)) {
                        ...
                        boolean urlMatch = driver.getCurrentUrl().contains("twitter.com") || 
                        ...
                        Assertions.assertTrue(urlMatch, "Social link URL should contain expected social domain.");
                        driver.close();
