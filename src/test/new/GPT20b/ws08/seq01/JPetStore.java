...
public void testLoginFlow() {
    ...
    Assumptions.assumeTrue(elementExists(signInLink),
    ...
    // Check for different possible login form element IDs
    By userIdLocator = By.id("username");
    if (!elementExists(userIdLocator)) {
        userIdLocator = By.id("userId");
    }
    if (!elementExists(userIdLocator)) {
        userIdLocator = By.name("username");
    }
    
    WebElement userField = wait.until(ExpectedConditions.visibilityOfElementLocated(userIdLocator));
    
    // Find password field with fallback
    By pwdLocator = By.id("password");
    if (!elementExists(pwdLocator)) {
        pwdLocator = By.name("password");
    }
    WebElement pwdField = driver.findElement(pwdLocator);
    
    // Find login button with fallback
    By loginBtnLocator = By.name("login");
    if (!elementExists(loginBtnLocator)) {
    ...
    // Verify presence of user menu - try different possible selectors
    By userMenuLocator = By.id("userMenu");
    if (!elementExists(userMenuLocator)) {
        userMenuLocator = By.xpath("//div[contains(@class,'user')]");
    }
    if (!elementExists(userMenuLocator)) {
        userMenuLocator = By.xpath("//span[contains(text(),'Welcome')]");
    }
    
    Assertions.assertTrue(
            elementExists(userMenuLocator),
...
public void testWithInvalidLogin() {
    ...
    // Check for error message - try different possible selectors
    By errorMsg = By.cssSelector(".error");
    if (!elementExists(errorMsg)) {
        errorMsg = By.cssSelector(".alert-error");
    }
    if (!elementExists(errorMsg)) {
        errorMsg = By.xpath("//div[contains(@class,'error')]");
    }
    if (!elementExists(errorMsg)) {
        errorMsg = By.xpath("//span[contains(text(),'Invalid')]");
    }
