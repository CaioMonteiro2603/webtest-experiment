...
public class JSFiddle {
    ...
    public void testEditorElementsPresent() {
        driver.get(BASE_URL);
        wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".codepanel")));
        Assertions.assertTrue(driver.findElements(By.cssSelector("[data-panel='HTML']")).size() > 0, "HTML editor should be present");
        Assertions.assertTrue(driver.findElements(By.cssSelector("[data-panel='CSS']")).size() > 0, "CSS editor should be present");
        Assertions.assertTrue(driver.findElements(By.cssSelector("[data-panel='JS']")).size() > 0, "JS editor should be present");
        Assertions.assertTrue(driver.findElements(By.cssSelector("[data-panel='Result']")).size() > 0, "Result panel should be present");
    }
    ...
    public void testNavbarLinks() {
        ...
        // Test Docs link
        WebElement docsLink = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(@href,'/docs')]")));
        docsLink.click();
        ...
        WebElement blogLink = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(@href,'blog')]")));
        blogLink.click();
    ...
    public void testAccountDropdown() {
        ...
        WebElement accountDropdown = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(@class,'dropdown')]")));
    ...
    public void testLoginFunctionality() {
        ...
        WebElement errorMessage = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".alert-danger, .error")));
        Assertions.assertTrue(errorMessage.getText().contains("Invalid") || errorMessage.getText().toLowerCase().contains("error"), "Should show invalid credentials error");
    }
}