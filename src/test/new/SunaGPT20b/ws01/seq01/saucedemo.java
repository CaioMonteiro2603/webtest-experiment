...
public class saucedemo{
    ...
    private void openMenu() {
        ...
        try {
            List<WebElement> menuOverlay = driver.findElements(By.className("bm-menu"));
            if (!menuOverlay.isEmpty() && menuOverlay.get(0).isDisplayed()) {
                WebElement closeButton = wait.until(ExpectedConditions.elementToBeClickable(By.id("react-burger-cross-btn")));
                closeButton.click();
                wait.until(ExpectedConditions.invisibilityOf(menuOverlay.get(0)));
            }
        } catch (Exception e) {
    ...
    public void testSortingDropdown() {
        login(USERNAME, PASSWORD);
        WebElement sortContainer = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".product_sort_container")));
    ...
    public void testFooterSocialLinks() {
        ...
        for (String[] pair : links) {
            String xpath = "//a[contains(@href,'" + pair[1] + "')]";
