...
public class conduit {
    ...
    private void login(String email, String password) {
        ...
        // Wait for home feed or user profile to appear after successful login
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div.feed-toggle")));
        } catch (Exception e) {
            // Alternative wait for user profile element
    ...
    public void testInvalidLoginShowsError() {
        ...
        // Expect an error message - try different selectors
        WebElement errorMsg = null;
        try {
            errorMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("ul.error-messages li")));
        } catch (Exception e) {
            // Try alternative error message selector
            errorMsg = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".error-messages")));
        }
        Assertions.assertTrue(errorMsg.getText().toLowerCase().contains("email") || errorMsg.getText().toLowerCase().contains("password") || errorMsg.getText().toLowerCase().contains("invalid"),
    ...
    public void testExternalFooterLinks() {
        ...
        // If no GitHub link found in footer, try finding it in the entire page
        if (githubLink == null) {
    ...
    public void testNavigationMenuItems() {
        ...
        // Wait for home page to load - try different selectors
        try {
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("div.feed-toggle")));
        } catch (Exception e) {
            // Alternative wait for home page content
            wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("nav.navbar")));
        }
