...
public class parabank {
    ...
    void testValidLogin() {
        ...
        wait.until(ExpectedConditions.urlMatches(".*overview\\.htm.*"));
    ...
    void testInvalidLoginCredentials() {
        ...
        assertTrue(errorMessage.getText().toLowerCase().contains("invalid") || 
                   errorMessage.getText().toLowerCase().contains("check") ||
                   errorMessage.getText().toLowerCase().contains("error"),
                "Error message should indicate invalid credentials");
        assertTrue(driver.getCurrentUrl().contains("login.htm") || driver.getCurrentUrl().contains("index.htm"), "Should remain on login page");
    }
    ...
    void testAccessAdminPageFromFooter() {
        ...
        wait.until(ExpectedConditions.titleIs("ParaBank | Admin Page"));
        assertTrue(driver.getCurrentUrl().contains("admin.htm"), "Should navigate to admin page");
        assertTrue(driver.getPageSource().contains("Administration"), "Admin page content should load");
    }
    ...
    void testNavigationMenuOptions() {
        ...
        assertTrue(driver.getPageSource().contains("Contact"), "Contact page should load");
    ...
    void testAccountServicesLinks() {
        ...
        assertTrue(driver.getPageSource().contains("Apply for a Loan"), "Loan Request page should load");
    ...
    void testLogoutFunctionality() {
        ...
        wait.until(ExpectedConditions.urlMatches(".*index\\.htm.*"));
        assertEquals("ParaBank | Welcome | Online Banking", driver.getTitle());
        assertTrue(driver.getPageSource().contains("Customer Login"), "Login form should be visible after logout");

        // Try to access protected page directly
        driver.get(BASE_URL.replace("index.htm", "overview.htm"));
        wait.until(ExpectedConditions.urlMatches(".*login\\.htm.*"));
        assertTrue(driver.getCurrentUrl().contains("login.htm"), "Should be redirected to login when accessing protected page");
    }
}