...
public class JPetStore {
	...
	private void verifyExternalLink(WebElement anchor) {
		...
		try {
			wait.until(d -> d.getWindowHandles().size() > before.size()
					|| !d.getCurrentUrl().equals(driver.getCurrentUrl()));
	...
	private void signIn(String user, String pass) {
		...
		if (signIn == null)
			signIn = first(By.partialLinkText("Sign"));
		if (signIn == null)
			signIn = first(By.cssSelector("a[href*='signon']"));
		...
		click(loginBtn != null ? loginBtn
				: driver.findElement(By.cssSelector("input[type='submit'], button[type='submit']")));
	...
	public void homePageLoadsAndHasCategories() {
		...
		boolean hasCategories = driver
				.findElements(By.cssSelector("area[href*='catalog/categories/'], a[href*='catalog/categories/']"))
				.size() > 0 || driver.findElements(By.cssSelector("div#Main, #Content, main")).size() > 0
	...
	public void validLoginAndLogoutIfAvailable() {
		...
		Assumptions.assumeTrue(success,
				"Could not confirm successful login with demo credentials; skipping assertions.");
		Assertions.assertTrue(
				driver.findElements(By.linkText("My Account")).size() > 0
		...
		Assertions.assertTrue(driver.findElements(By.linkText("Sign In")).size() > 0,
				"Sign In link should reappear after logout.");
	}
	...
	public void invalidLoginShowsError() {
		...
		boolean hasError = driver.findElements(By.xpath(
				"//*[contains(translate(text(),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'invalid') or contains(translate(text(),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'signon failed')]"))
				.size() > 0;
		Assumptions.assumeTrue(hasError || !isSignedIn(),
				"No explicit error found but not signed in; treating as negative case.");
	...
	public void browseCategoryProductItemAndAddToCart() {
		...
		if (fish == null)
			fish = first(By.cssSelector("area[href*='catalog/categories/FISH']"));
		WebElement anyCat = fish != null ? fish
				: first(By.cssSelector("a[href*='catalog/categories/'], area[href*='catalog/categories/']"));
		if (anyCat == null)
			anyCat = first(By.tagName("area"));
	...
	public void searchForFishAndOptionallySort() {
		...
		if (input == null)
			input = first(By.cssSelector("input[type='text']"));
		...
		if (searchBtn == null)
			searchBtn = first(By.xpath("//input[@type='submit' and @value='Search']"));
		if (searchBtn == null)
			searchBtn = first(By.xpath("//button[contains(.,'Search')]"));
		if (searchBtn == null)
			searchBtn = first(By.cssSelector("input[type='submit']"));
		Assertions.assertNotNull(searchBtn, "Search submit control should exist.");
		click(searchBtn);

		// Verify results
		wait.until(ExpectedConditions.presenceOfElementLocated(By.tagName("body")));
		boolean hasResults = driver.findElements(By.cssSelector("a[href*='catalog/products/']")).size() > 0 || driver
	...
	private String firstResultText() {
		List<WebElement> results = driver
				.findElements(By.cssSelector("a[href*='catalog/products/'], .result, .item, article"));
		if (!results.isEmpty()) {
	...
	public void footerSocialLinksAsExternalIfPresent() {
		...
		List<String> domains = Arrays.asList("twitter.com", "facebook.com", "linkedin.com", "github.com",
				"youtube.com");
