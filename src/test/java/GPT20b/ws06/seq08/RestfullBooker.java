...
public class RestfullBooker {
    ...
    public void testExternalLinksPolicy() {
        navigateToBase();
        List<WebElement> links = driver.findElements(By.xpath("//a[@href and not(contains(@href, '" + BASE_HOST + "')) and (contains(@href, 'http://') or contains(@href, 'https://'))]"));
    ...
    public void testFooterSocialLinks() {
        navigateToBase();
        List<WebElement> socialLinks = driver.findElements(By.xpath("//a[contains(@href,'twitter.') or contains(@href,'facebook.') or contains(@href,'linkedin.')]"));
        Assumptions.assumeTrue(!socialLinks.isEmpty(), "Social links not found, test skipped");
                ...
                for (String handle : driver.getWindowHandles()) {
                    if (!handle.equals(originalWindow)) {
                        ...
                        Assertions.assertTrue(currentUrl.contains("twitter") || currentUrl.contains("facebook") || currentUrl.contains("linkedin"),
                                "Social link URL should contain expected domain");
                        driver.close();
                        driver.switchTo().window(originalWindow);
                        break;
                    }
