...
public class DemoAUT {
    ...
    public void testValidFormSubmissionShowsSuccess() {
        ...
        // Check for success message or successful form submission indicators
        boolean successFound = false;
        
        // Check multiple possible success indicators
        if (!driver.findElements(By.cssSelector(".alert-success, .success, .form-success")).isEmpty()) {
            successFound = true;
        } else if (!driver.findElements(By.cssSelector("div:contains('Thank you')")).isEmpty()) {
            successFound = true;
        } else if (!driver.findElements(By.cssSelector("p:contains('Thank you')")).isEmpty()) {
            successFound = true;
        } else if (!driver.findElements(By.cssSelector("*:contains('submitted successfully')")).isEmpty()) {
            successFound = true;
        } else {
            // Check if form disappeared or URL changed
            try {
                driver.findElement(By.cssSelector("form"));
                // Form still exists, check if it's in a submitted state
                List<WebElement> inputs = driver.findElements(By.cssSelector("input[type='text'], input[type='email'], textarea"));
                boolean allEmpty = true;
                for (WebElement input : inputs) {
                    if (input.getAttribute("value") != null && !input.getAttribute("value").isEmpty()) {
                        allEmpty = false;
                        break;
                    }
                }
                ...
                // Form no longer exists, likely submission was successful
                successFound = true;
            }
        }
        
        assertTrue(successFound, "Form submission should result in success message or successful state change");
    }
