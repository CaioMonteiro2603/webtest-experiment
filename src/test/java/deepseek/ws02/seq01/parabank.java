...
public class parabank {
    ...
    public void testLogin() {
        ...
        WebElement errorMessage = driver.findElements(By.cssSelector("p.error")).size() > 0 ? 
            driver.findElement(By.cssSelector("p.error")) : null;
        if (errorMessage != null && errorMessage.isDisplayed()) {
            Assertions.assertTrue(errorMessage.getText().contains("Error"), "Should show login error for invalid credentials");
        } else {
    ...
    public void testInvalidLogin() {
        ...
        WebElement errorMessage = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div[class*='error'] p, p.error")));
        Assertions.assertTrue(errorMessage.getText().contains("Error") || errorMessage.getText().contains("error"), 
    ...
    public void testAccountServicesNavigation() {
        ...
        Assertions.assertTrue(driver.findElement(By.tagName("h1")).getText().contains("Bill Pay"), "Should navigate to Bill Pay page");
    }
        ...
        if (driver.getCurrentUrl().contains("index.htm") && driver.findElements(By.name("username")).size() > 0) {
            ...
            try {
                ...
                if (driver.getCurrentUrl().contains("login.htm")) {
                    Assertions.fail("Login failed with provided credentials");
                }
            }
        } else if (driver.getCurrentUrl().contains("login.htm")) {
            WebElement usernameField = wait.until(ExpectedConditions.presenceOfElementLocated(By.name("username")));
            WebElement passwordField = driver.findElement(By.name("password"));
            WebElement loginButton = driver.findElement(By.cssSelector("input[value='Log In']"));
