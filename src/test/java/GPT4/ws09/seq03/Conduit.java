...
public class RealWorldTest {
    ...
    public void testNavigationLinks() {
        ...
        for (WebElement link : navLinks) {
            Assertions.assertTrue(link.isDisplayed(), "Navigation link not displayed: " + link.getText());
        }
    }
    ...
    public void testGlobalFeedLoads() {
        WebElement globalFeedTab = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("a.nav-link.active")));
        Assertions.assertTrue(globalFeedTab.getText().contains("Global Feed"), "Global Feed tab not active by default");
    ...
    public void testArticleNavigation() {
        List<WebElement> articles = driver.findElements(By.cssSelector(".article-preview h1"));
        Assertions.assertTrue(articles.size() > 0, "No article titles found");
        String title = articles.get(0).getText();
